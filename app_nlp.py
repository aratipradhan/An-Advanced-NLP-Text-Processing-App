...
text1= st.text_area("Enter your text here to see  bigrams and trigrams")
...
# N-grams
n = st.number_input("Choose n for N-grams", min_value=1, max_value=5, value=2)
...
# Stemming Options
text2=st.text_area("Enter your word here which you want to stem")
if st.button("PorterStemmer"):
    ps = PorterStemmer()
    stemmed_words = [ps.stem(word) for word in word_tokenize(text2)]
    st.write("Porter Stemmed Words:")
    st.write(stemmed_words)

if st.button("LancasterStemmer"):
    ls = LancasterStemmer()
    stemmed_words = [ls.stem(word) for word in word_tokenize(text2)]
    st.write("Lancaster Stemmed Words:")
    st.write(stemmed_words)

if st.button("SnowballStemmer"):
    ss = SnowballStemmer("english")
    stemmed_words = [ss.stem(word) for word in word_tokenize(text2)]
...
if st.button("WordNet Lemmatizer"):
    ...
    lemmatized_words = [lemmatizer.lemmatize(word) for word in word_tokenize(text2)]
...
text3=st.text_area("Enter a list of word")
...
text4=st.text_area("Enter your text here which you want to display")
